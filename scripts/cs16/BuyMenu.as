...
void BuyKevlar( CBasePlayer@ pPlayer )
{
    int idx = pPlayer.entindex();
    if( idx < 1 || idx > 32 ) return;

    const uint uiCost = 650;
    string pid = g_EngineFuncs.GetPlayerAuthId( pPlayer.edict() );

    if( uint( BuyPoints[pid] ) < uiCost )
    {
        g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CS16 BUYMENU] Not enough money for Kevlar - Cost: $650\n" );
        return;
    }

    BuyPoints[pid] = uint( BuyPoints[pid] ) - uiCost;
    CS16BASE::g_PlayerArmor[idx] = 100;
    BuyMenuCVARS@ hooks = @BuyMenuCVARS();
    hooks.ShowPointsSprite( pPlayer );
    g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CS16 BUYMENU] Kevlar purchased (100 armor)\n" );
}

void BuyHelmet( CBasePlayer@ pPlayer )
{
    int idx = pPlayer.entindex();
    if( idx < 1 || idx > 32 ) return;

    if( CS16BASE::g_PlayerArmor[idx] <= 0 )
    {
        g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CS16 BUYMENU] Buy Kevlar first!\n" );
        return;
    }
    if( CS16BASE::g_PlayerHelmet[idx] )
    {
        g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CS16 BUYMENU] You already have a helmet!\n" );
        return;
    }

    const uint uiCost = 350;
    string pid = g_EngineFuncs.GetPlayerAuthId( pPlayer.edict() );

    if( uint( BuyPoints[pid] ) < uiCost )
    {
        g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CS16 BUYMENU] Not enough money for Helmet - Cost: $350\n" );
        return;
    }

    BuyPoints[pid] = uint( BuyPoints[pid] ) - uiCost;
    CS16BASE::g_PlayerHelmet[idx] = true;
    BuyMenuCVARS@ hooks = @BuyMenuCVARS();
    hooks.ShowPointsSprite( pPlayer );
    g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CS16 BUYMENU] Helmet purchased\n" );
}
...
final class BuyMenuCVARS
{
    ...
    HookReturnCode CS16_ClientSay( SayParameters@ pParams )
    {
        ...
        {
            if( args.Arg(1).ToLowercase() == "kevlar" )
            {
                pParams.ShouldHide = true;
                BuyKevlar( pPlayer );
                return HOOK_CONTINUE;
            }
            else if( args.Arg(1).ToLowercase() == "helmet" )
            {
                pParams.ShouldHide = true;
                BuyHelmet( pPlayer );
                return HOOK_CONTINUE;
            }
            else if( args.Arg(1).ToLowercase() == "ammo" )
            {
                pParams.ShouldHide = true;
                bool bItemFound = false;
                string szItemName;
...
void RegisterBuyMenuCCVars()
{
    ...
    {
        //g_Game.AlertMessage( at_notice, "CCVars added\n" );
...
// Call once from PluginInit() — never from MapInit().
void RegisterHooks()
{
...
final class BuyMenu
{
    ...
    private void CreateMenu()
    {
        ...
        m_pEquipMenu.AddItem( "$650 Kevlar Vest" );
        m_pEquipMenu.AddItem( "$350 Helmet (requires Kevlar)" );
    ...
    private void EquipCallback( CTextMenu@ menu, CBasePlayer@ pPlayer, int iSlot, const CTextMenuItem@ pItem )
    {
        if( pItem !is null )
        {
            // Handle armor items (no BuyableItem backing — pure state)
            if( pItem.m_szName == "$650 Kevlar Vest" )
            {
                BuyKevlar( pPlayer );
                menu.Open( 0, 0, pPlayer );
                return;
            }
            if( pItem.m_szName == "$350 Helmet (requires Kevlar)" )
            {
                BuyHelmet( pPlayer );
                menu.Open( 0, 0, pPlayer );
                return;
