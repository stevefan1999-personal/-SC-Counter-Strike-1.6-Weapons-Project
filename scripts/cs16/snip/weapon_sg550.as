...
int AIM_SPEED       = 200;
float ACCURACY_START  = 0.98f;
float ACCURACY_DECAY  = 0.2f;
float ACCURACY_MIN    = 0.65f;
float ACCURACY_MAX    = 0.98f;
...
class weapon_sg550 : ScriptBasePlayerWeaponEntity, CS16BASE::WeaponBase
{
    ...
    void PrimaryAttack()
    {
        ...
        SG550Fire( flSpread, RPM, false );
    }

    void SG550Fire( float flSpread, float flCycleTime, bool fUseAutoAim )
    {
        float flTimeSinceFire = g_Engine.time - m_flLastFireTime;
        m_flAccuracy = Math.min( flTimeSinceFire * 0.35f + 0.65f, 0.98f );
