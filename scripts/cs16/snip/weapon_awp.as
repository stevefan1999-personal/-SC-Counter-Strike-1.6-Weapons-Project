// Counter-Strike 1.6 Magnum Sniper Rifle (AWP)
/* Model Credits
/ Model: Valve
/ Textures: Valve
/ Animations: Valve
...
// Models
string W_MODEL      = "models/cs16/wpn/awp/w_awp.mdl";
string V_MODEL      = "models/cs16/wpn/awp/v_awp.mdl";
string P_MODEL      = "models/cs16/wpn/awp/p_awp.mdl";
...
string SHOOT_S      = "cs16/awp/shoot.wav";
// Information
int MAX_CARRY       = 300;
int MAX_CLIP        = 10;
...
class weapon_awp : ScriptBasePlayerWeaponEntity, CS16BASE::WeaponBase
{
    ...
    void AWPFire( float flSpread, float flCycleTime, bool fUseAutoAim )
    {
        ...
        if( WeaponZoomMode != CS16BASE::MODE_FOV_NORMAL && self.m_iClip > 0 )
        {
            ...
            self.pev.nextthink = g_Engine.time + flCycleTime + 0.1f;
            self.m_flTimeWeaponIdle = WeaponTimeBase() + flCycleTime + 0.1f;
        }
        else
        ...
        KickBack( 2.0, 1.0, 0.0, 0.0, 2.0, 1.0, 0 );
...
class AWP_MAG : ScriptBasePlayerAmmoEntity, CS16BASE::AmmoBase
{
    void Spawn()
    {
        Precache();
...
void Register()
{
    CS16BASE::RegisterCWEntity( "CS16_AWP::", "weapon_awp", GetName(), GetAmmoName(), "AWP_MAG", 
