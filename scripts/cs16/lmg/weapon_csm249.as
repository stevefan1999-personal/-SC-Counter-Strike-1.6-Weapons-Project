...
float ACCURACY_DIV    = 175.0f;
float ACCURACY_EXP    = 3.0f;
float ACCURACY_BASE   = 0.4f;
float ACCURACY_CAP    = 0.9f;
...
class weapon_csm249 : ScriptBasePlayerWeaponEntity, CS16BASE::WeaponBase
{
    ...
    void PrimaryAttack()
    {
        ...
        if( !( m_pPlayer.pev.flags & FL_ONGROUND != 0 ) )
            M249Fire( 0.045f + 0.5f * m_flAccuracy, RPM, false );
        else if( m_pPlayer.pev.velocity.Length2D() > 140 )
            M249Fire( 0.045f + 0.095f * m_flAccuracy, RPM, false );
        else
            M249Fire( 0.03f * m_flAccuracy, RPM, false );
    }
    ...
    void M249Fire( float flSpread, float flCycleTime, bool fUseAutoAim )
    {
        ...
        if( !( m_pPlayer.pev.flags & FL_ONGROUND != 0 ) )
        {
            KickBack( 1.8, 0.65, 0.45, 0.125, 5.5, 4.0, 5 );
        }
        else if( m_pPlayer.pev.velocity.Length2D() > 0 )
        {
            KickBack( 1.1, 0.5, 0.3, 0.06, 4.5, 2.5, 7 );
        }
        else if( m_pPlayer.pev.flags & FL_DUCKING != 0 )
        {
            KickBack( 0.75, 0.325, 0.25, 0.025, 4.25, 1.5, 9 );
        }
        else
        {
            KickBack( 0.8, 0.35, 0.3, 0.03, 4.5, 1.75, 8 );
        }
