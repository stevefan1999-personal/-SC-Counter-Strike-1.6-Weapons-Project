...
class weapon_xm1014 : ScriptBasePlayerWeaponEntity, CS16BASE::WeaponBase
{
    ...
    void PrimaryAttack()
    {
        ...
        XM1014Fire( 0.0725f, RPM, false );
    }

    void XM1014Fire( float flSpread, float flCycleTime, bool fUseAutoAim )
    {
        ...
        ShootWeapon( SHOOT_S, PELLETS, vecSpread, MAX_SHOOT_DIST, DAMAGE, DMG_LAUNCH );

        if( !( m_pPlayer.pev.flags & FL_ONGROUND != 0 ) )
            m_pPlayer.pev.punchangle.x = Math.RandomFloat( -7.0f, -10.0f );
        else
            m_pPlayer.pev.punchangle.x = Math.RandomFloat( -3.0f, -5.0f );
