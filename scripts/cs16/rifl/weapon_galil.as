...
float ACCURACY_DIV    = 200.0f;
float ACCURACY_EXP    = 3.0f;
float ACCURACY_BASE   = 0.35f;
float ACCURACY_CAP    = 1.25f;
...
class weapon_galil : ScriptBasePlayerWeaponEntity, CS16BASE::WeaponBase
{
    ...
    void PrimaryAttack()
    {
        ...
        if( !( m_pPlayer.pev.flags & FL_ONGROUND != 0 ) )
            GalilFire( 0.04f + 0.3f * m_flAccuracy, RPM, false );
        else if( m_pPlayer.pev.velocity.Length2D() > 140 )
            GalilFire( 0.04f + 0.07f * m_flAccuracy, RPM, false );
        else
            GalilFire( 0.0375f * m_flAccuracy, RPM, false );
    }
