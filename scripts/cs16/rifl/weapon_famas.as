...
class weapon_famas : ScriptBasePlayerWeaponEntity, CS16BASE::WeaponBase
{
    ...
    void FAMASFire( float flSpread, float flCycleTime, bool fUseAutoAim )
    {
        ++m_iShotsFired;
    ...
    private void FireWeapon()
    {
        float flSpread;
        if( !( m_pPlayer.pev.flags & FL_ONGROUND != 0 ) )
            flSpread = 0.030f + 0.3f * m_flAccuracy;
        else if( m_pPlayer.pev.velocity.Length2D() > 140 )
            flSpread = 0.030f + 0.07f * m_flAccuracy;
        else
            flSpread = 0.02f * m_flAccuracy;

        FAMASFire( flSpread, 0.0f, false );
    }
