...
class weapon_m4a1 : ScriptBasePlayerWeaponEntity, CS16BASE::WeaponBase
{
    ...
    void PrimaryAttack()
    {
        ...
        if( !( m_pPlayer.pev.flags & FL_ONGROUND != 0 ) )
            M4A1Fire( 0.035f + 0.4f * m_flAccuracy, RPM, false );
        else if( m_pPlayer.pev.velocity.Length2D() > 140 )
            M4A1Fire( 0.035f + 0.07f * m_flAccuracy, RPM, false );
        else
            M4A1Fire( ( WeaponSilMode == CS16BASE::MODE_SUP_ON ) ? 0.025f * m_flAccuracy : 0.02f * m_flAccuracy, RPM, false );
    }
    ...
    void M4A1Fire( float flSpread, float flCycleTime, bool fUseAutoAim )
    {
        ...
        if( !( m_pPlayer.pev.flags & FL_ONGROUND != 0 ) )
        {
            KickBack( 1.2, 0.5, 0.23, 0.15, 5.5, 3.5, 5 );
        }
        else if( m_pPlayer.pev.velocity.Length2D() > 0 )
        {
            KickBack( 1.0, 0.45, 0.28, 0.045, 5.0, 2.5, 7 );
        }
        else if( m_pPlayer.pev.flags & FL_DUCKING != 0 )
        {
            KickBack( 0.6, 0.3, 0.2, 0.0125, 4.75, 1.5, 9 );
        }
        else
        {
            KickBack( 0.65, 0.35, 0.25, 0.015, 5.0, 1.75, 8 );
        }
