...
float ACCURACY_START  = 0.2f;
float ACCURACY_DIV    = 220.0f;
float ACCURACY_EXP    = 3.0f;
float ACCURACY_BASE   = 0.3f;
float ACCURACY_CAP    = 1.0f;
int AIM_SPEED       = 270;
...
class weapon_sg552 : ScriptBasePlayerWeaponEntity, CS16BASE::WeaponBase
{
    ...
    void PrimaryAttack()
    {
        ...
        float flCycleTime = (WeaponZoomMode != CS16BASE::MODE_FOV_NORMAL) ? RPM_ZOOMED : RPM;

        if( !( m_pPlayer.pev.flags & FL_ONGROUND != 0 ) )
            SG552Fire( 0.035f + 0.45f * m_flAccuracy, flCycleTime, false );
        else if( m_pPlayer.pev.velocity.Length2D() > 140 )
            SG552Fire( 0.035f + 0.075f * m_flAccuracy, flCycleTime, false );
        else
            SG552Fire( 0.02f * m_flAccuracy, flCycleTime, false );
    }

    void SG552Fire( float flSpread, float flCycleTime, bool fUseAutoAim )
    {
        ++m_iShotsFired;
        UpdateAccuracyBloom( ACCURACY_DIV, ACCURACY_EXP, ACCURACY_BASE, ACCURACY_CAP );
        ...
        if( !( m_pPlayer.pev.flags & FL_ONGROUND != 0 ) )
        {
            KickBack( 1.25, 0.45, 0.22, 0.18, 5.0, 3.0, 5 );
        }
        else if( m_pPlayer.pev.velocity.Length2D() > 0 )
        {
            KickBack( 1.0, 0.45, 0.28, 0.04, 5.0, 2.5, 7 );
        }
        else if( m_pPlayer.pev.flags & FL_DUCKING != 0 )
        {
            KickBack( 0.6, 0.35, 0.2, 0.0125, 4.75, 1.5, 9 );
        }
        else
        {
            KickBack( 0.625, 0.375, 0.25, 0.0125, 5.0, 1.75, 8 );
        }
