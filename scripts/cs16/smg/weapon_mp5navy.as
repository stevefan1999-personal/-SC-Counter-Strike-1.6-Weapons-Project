...
// Accuracy
float ACCURACY_START = 0.0f;
float ACCURACY_DIV   = 220.1f;
float ACCURACY_EXP   = 2.0f;
float ACCURACY_BASE  = 0.45f;
float ACCURACY_CAP   = 0.75f;
...
class weapon_mp5navy : ScriptBasePlayerWeaponEntity, CS16BASE::WeaponBase
{
    ...
    void PrimaryAttack()
    {
        ...
        if( !( m_pPlayer.pev.flags & FL_ONGROUND != 0 ) )
            MP5Fire( 0.04f + 0.3f * m_flAccuracy, RPM, false );
        else if( m_pPlayer.pev.velocity.Length2D() > 0 )
            MP5Fire( 0.04f + 0.07f * m_flAccuracy, RPM, false );
        else
            MP5Fire( 0.04f * m_flAccuracy, RPM, false );
    }
    ...
    void MP5Fire( float flSpread, float flCycleTime, bool fUseAutoAim )
    {
        ...
        if( !( m_pPlayer.pev.flags & FL_ONGROUND != 0 ) )
        {
            KickBack( 0.9, 0.475, 0.35, 0.0425, 3.5, 1.5, 7 );
        }
        else if( m_pPlayer.pev.velocity.Length2D() > 0 )
        {
            KickBack( 0.5, 0.275, 0.2, 0.03, 3.0, 1.0, 9 );
        }
        else if( m_pPlayer.pev.flags & FL_DUCKING != 0 )
        {
            KickBack( 0.225, 0.15, 0.1, 0.015, 2.5, 0.75, 9 );
        }
        else
        {
            KickBack( 0.25, 0.175, 0.125, 0.02, 2.75, 1.0, 8 );
        }
