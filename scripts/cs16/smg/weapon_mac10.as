...
// Accuracy
float ACCURACY_START = 0.15f;
float ACCURACY_DIV   = 200.0f;
float ACCURACY_EXP   = 3.0f;
float ACCURACY_BASE  = 0.6f;
float ACCURACY_CAP   = 1.65f;
...
class weapon_mac10 : ScriptBasePlayerWeaponEntity, CS16BASE::WeaponBase
{
    ...
    void PrimaryAttack()
    {
        ...
        if( !( m_pPlayer.pev.flags & FL_ONGROUND != 0 ) )
            MAC10Fire( 0.375f * m_flAccuracy, RPM, false );
        else
            MAC10Fire( 0.03f * m_flAccuracy, RPM, false );
    }
    ...
    void MAC10Fire( float flSpread, float flCycleTime, bool fUseAutoAim )
    {
        ...
        if( !( m_pPlayer.pev.flags & FL_ONGROUND != 0 ) )
        {
            KickBack( 1.3, 0.55, 0.4, 0.05, 4.0, 2.5, 7 );
        }
        else if( m_pPlayer.pev.velocity.Length2D() > 0 )
        {
            KickBack( 0.9, 0.45, 0.25, 0.035, 3.5, 1.5, 9 );
        }
        else if( m_pPlayer.pev.flags & FL_DUCKING != 0 )
        {
            KickBack( 0.75, 0.4, 0.175, 0.03, 3.0, 1.0, 9 );
        }
        else
        {
            KickBack( 0.775, 0.425, 0.2, 0.03, 3.25, 1.25, 8 );
        }
