...
float ACCURACY_DIV   = 210.0f;
float ACCURACY_EXP   = 2.0f;
float ACCURACY_BASE  = 0.5f;
...
class weapon_ump45 : ScriptBasePlayerWeaponEntity, CS16BASE::WeaponBase
{
    ...
    void PrimaryAttack()
    {
        ...
        if( !( m_pPlayer.pev.flags & FL_ONGROUND != 0 ) )
            UMP45Fire( 0.24f * m_flAccuracy, RPM, false );
        else
            UMP45Fire( 0.04f * m_flAccuracy, RPM, false );
    }
    ...
    void UMP45Fire( float flSpread, float flCycleTime, bool fUseAutoAim )
    {
        ...
        if( !( m_pPlayer.pev.flags & FL_ONGROUND != 0 ) )
        {
            KickBack( 0.125, 0.65, 0.55, 0.0475, 3.25, 2.0, 7 );
        }
        else if( m_pPlayer.pev.velocity.Length2D() > 0 )
        {
            KickBack( 0.55, 0.3, 0.225, 0.03, 3.0, 1.25, 9 );
        }
        else if( m_pPlayer.pev.flags & FL_DUCKING != 0 )
        {
            KickBack( 0.25, 0.175, 0.125, 0.02, 2.5, 0.75, 9 );
        }
        else
        {
            KickBack( 0.275, 0.2, 0.15, 0.0225, 2.75, 1.0, 8 );
        }
