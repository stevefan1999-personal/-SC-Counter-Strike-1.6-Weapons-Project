...
float ACCURACY_BASE  = 0.55f;
float ACCURACY_CAP   = 1.4f;
...
class weapon_tmp : ScriptBasePlayerWeaponEntity, CS16BASE::WeaponBase
{
    ...
    void PrimaryAttack()
    {
        ...
        if( !( m_pPlayer.pev.flags & FL_ONGROUND != 0 ) )
            TMPFire( 0.25f * m_flAccuracy, RPM, false );
        else
            TMPFire( 0.03f * m_flAccuracy, RPM, false );
    }
    ...
    void TMPFire( float flSpread, float flCycleTime, bool fUseAutoAim )
    {
        ...
        if( !( m_pPlayer.pev.flags & FL_ONGROUND != 0 ) )
        {
            KickBack( 1.1, 0.5, 0.35, 0.045, 3.5, 2.0, 7 );
        }
        else if( m_pPlayer.pev.velocity.Length2D() > 0 )
        {
            KickBack( 0.8, 0.4, 0.2, 0.03, 3.25, 1.25, 9 );
        }
        else if( m_pPlayer.pev.flags & FL_DUCKING != 0 )
        {
            KickBack( 0.7, 0.35, 0.125, 0.025, 3.0, 0.875, 9 );
        }
        else
        {
            KickBack( 0.725, 0.375, 0.15, 0.025, 3.25, 1.0, 8 );
        }
