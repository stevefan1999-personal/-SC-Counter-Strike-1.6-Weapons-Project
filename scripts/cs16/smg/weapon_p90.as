...
class weapon_p90 : ScriptBasePlayerWeaponEntity, CS16BASE::WeaponBase
{
    ...
    void PrimaryAttack()
    {
        ...
        if( !( m_pPlayer.pev.flags & FL_ONGROUND != 0 ) )
            P90Fire( 0.3f * m_flAccuracy, RPM, false );
        else if( m_pPlayer.pev.velocity.Length2D() > 0 )
            P90Fire( 0.115f * m_flAccuracy, RPM, false );
        else
            P90Fire( 0.045f * m_flAccuracy, RPM, false );
    }
    ...
    void P90Fire( float flSpread, float flCycleTime, bool fUseAutoAim )
    {
        ...
        if( !( m_pPlayer.pev.flags & FL_ONGROUND != 0 ) )
        {
            KickBack( 0.9, 0.45, 0.35, 0.04, 3.0, 1.5, 7 );
        }
        else if( m_pPlayer.pev.velocity.Length2D() > 0 )
        {
            KickBack( 0.45, 0.3, 0.2, 0.0275, 2.75, 1.0, 9 );
        }
        else if( m_pPlayer.pev.flags & FL_DUCKING != 0 )
        {
            KickBack( 0.275, 0.2, 0.125, 0.02, 2.5, 0.75, 9 );
        }
        else
        {
            KickBack( 0.3, 0.225, 0.125, 0.02, 2.75, 1.0, 8 );
        }
