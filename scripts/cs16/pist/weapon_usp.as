...
class weapon_usp : ScriptBasePlayerWeaponEntity, CS16BASE::WeaponBase
{
    ...
    void PrimaryAttack()
    {
        ...
        bool bSilenced = (WeaponSilMode == CS16BASE::MODE_SUP_ON);

        if( !( m_pPlayer.pev.flags & FL_ONGROUND != 0 ) )
            USPFire( bSilenced ? 1.3f * flInvAcc : 1.2f * flInvAcc, RPM, false );
        else if( m_pPlayer.pev.velocity.Length2D() > 0 )
            USPFire( bSilenced ? 0.25f * flInvAcc : 0.225f * flInvAcc, RPM, false );
        else if( m_pPlayer.pev.flags & FL_DUCKING != 0 )
            USPFire( bSilenced ? 0.125f * flInvAcc : 0.08f * flInvAcc, RPM, false );
        else
            USPFire( bSilenced ? 0.15f * flInvAcc : 0.1f * flInvAcc, RPM, false );
    }

    void USPFire( float flSpread, float flCycleTime, bool fUseAutoAim )
    {
        ++m_iShotsFired;
        UpdateAccuracyDecay( 0.3f, 0.275f, 0.6f, 0.92f );
