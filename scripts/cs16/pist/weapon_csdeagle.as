...
float MOVING_BASE     = 0.25f;
float DUCKING_BASE    = 0.115f;
float STANDING_BASE   = 0.13f;
...
class weapon_csdeagle : ScriptBasePlayerWeaponEntity, CS16BASE::WeaponBase
{
    ...
    void PrimaryAttack()
    {
        ...
        if( !( m_pPlayer.pev.flags & FL_ONGROUND != 0 ) )
            DEAGLEFire( AIRBORNE_BASE * flInvAcc, RPM, false );
        else if( m_pPlayer.pev.velocity.Length2D() > 0 )
            DEAGLEFire( MOVING_BASE * flInvAcc, RPM, false );
        else if( m_pPlayer.pev.flags & FL_DUCKING != 0 )
            DEAGLEFire( DUCKING_BASE * flInvAcc, RPM, false );
        else
            DEAGLEFire( STANDING_BASE * flInvAcc, RPM, false );
    }

    void DEAGLEFire( float flSpread, float flCycleTime, bool fUseAutoAim )
    {
        ++m_iShotsFired;
        UpdateAccuracyDecay( 0.4f, 0.35f, 0.55f, 0.9f );
