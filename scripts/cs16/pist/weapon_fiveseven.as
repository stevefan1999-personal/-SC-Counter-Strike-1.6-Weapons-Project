...
// Accuracy
float ACCURACY_START  = 0.92f;
float ACCURACY_DECAY  = 0.35f;
float ACCURACY_MIN    = 0.725f;
float ACCURACY_MAX    = 0.92f;
float AIRBORNE_BASE   = 1.5f;
float MOVING_BASE     = 0.255f;
float DUCKING_BASE    = 0.075f;
float STANDING_BASE   = 0.15f;
...
class weapon_fiveseven : ScriptBasePlayerWeaponEntity, CS16BASE::WeaponBase
{
    ...
    void PrimaryAttack()
    {
        ...
        if( !( m_pPlayer.pev.flags & FL_ONGROUND != 0 ) )
            FiveSevenFire( AIRBORNE_BASE * flInvAcc, RPM, false );
        else if( m_pPlayer.pev.velocity.Length2D() > 0 )
            FiveSevenFire( MOVING_BASE * flInvAcc, RPM, false );
        else if( m_pPlayer.pev.flags & FL_DUCKING != 0 )
            FiveSevenFire( DUCKING_BASE * flInvAcc, RPM, false );
        else
            FiveSevenFire( STANDING_BASE * flInvAcc, RPM, false );
    }

    void FiveSevenFire( float flSpread, float flCycleTime, bool fUseAutoAim )
    {
        ++m_iShotsFired;
        UpdateAccuracyDecay( 0.275f, 0.25f, 0.725f, 0.92f );
