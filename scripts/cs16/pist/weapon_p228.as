...
class weapon_p228 : ScriptBasePlayerWeaponEntity, CS16BASE::WeaponBase
{
    ...
    void PrimaryAttack()
    {
        ...
        if( !( m_pPlayer.pev.flags & FL_ONGROUND != 0 ) )
            P228Fire( AIRBORNE_BASE * flInvAcc, RPM, false );
        else if( m_pPlayer.pev.velocity.Length2D() > 0 )
            P228Fire( MOVING_BASE * flInvAcc, RPM, false );
        else if( m_pPlayer.pev.flags & FL_DUCKING != 0 )
            P228Fire( DUCKING_BASE * flInvAcc, RPM, false );
        else
            P228Fire( STANDING_BASE * flInvAcc, RPM, false );
    }

    void P228Fire( float flSpread, float flCycleTime, bool fUseAutoAim )
    {
        ++m_iShotsFired;
        UpdateAccuracyDecay( 0.325f, 0.3f, 0.6f, 0.9f );
